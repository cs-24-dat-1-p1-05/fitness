...
int main() {
    ...
    do {
        ...
        if (user_choice == 1) {
            ...
            user_t user = returning_user(name);


            if (user.recommendation_id > 0) {
                printf("Recommendation ID: %d\n", user.recommendation_id);
                display_recommendation(user.recommendation_id);
                ...
                adjust_recommendation(&user, feedback);

                update_user_file(&user, user.recommendation_id);
